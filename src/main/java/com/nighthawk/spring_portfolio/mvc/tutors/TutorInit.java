...
public class TutorInit {
    ...
    @Bean
    CommandLineRunner runPractices() { // The run() method will be executed after the application starts
        return args -> {
            ...
            for (String teachers : problemsArray) {
                List<Tutor> test = repository.findByTeacherIgnoreCase(teachers); // JPA lookup
                if (test.size() == 0)
                    repository.save(new Tutor(null, teachers, 1, "1.1 Newton's Laws",
