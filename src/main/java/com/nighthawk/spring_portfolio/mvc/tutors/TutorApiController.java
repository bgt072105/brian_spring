...
public class TutorApiController {
     ...
     */
    @GetMapping("/")
    public ResponseEntity<List<Tutor>> getExperience() {
     ...
     */
    @PostMapping("/add")
    public ResponseEntity<Tutor> addExperience(@RequestParam("experience") String experience,
            @RequestParam("Age") int Age,
            @RequestParam("Email") String Email,
            @RequestParam("Review") String Review) {
        repository.save(new Tutor(null, experience, Age, Email, Review)); // JPA save
        long maxId = repository.getMaxId();
        Optional<Tutor> optional = repository.findById(maxId);
        if (optional.isPresent()) {
            Tutor information = optional.get();
     ...
     */
    @GetMapping("/search")
    public ResponseEntity<List<Tutor>> getPerson(@RequestParam("email") String email) {
        return new ResponseEntity<>(repository.findByExperienceOrReview(email), HttpStatus.OK);
    }
