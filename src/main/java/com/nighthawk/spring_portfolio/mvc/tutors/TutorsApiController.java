...
public class TutorsApiController {
     ...
     */
    @PutMapping("/like/{id}")
    public ResponseEntity<Tutors> setLike(@PathVariable String id) {
        /*
     ...
     */
    @PutMapping("/jeer/{id}")
    public ResponseEntity<Tutors> setJeer(@PathVariable String id) {
        Optional<Tutors> optional = repository.findById(id);
        if (optional.isPresent()) { // Good ID
    ...
    @PutMapping("/share/{id}")
    public ResponseEntity<Tutors> setTags(@PathVariable String id) {
        Optional<Tutors> optional = repository.findById(id);
        if (optional.isPresent()) { // Good ID
            Tutors tutor = optional.get();
            tutor.setPastReviews(tutor.getPastReviews() + 1);
            repository.save(tutor);
            return new ResponseEntity<>(tutor, HttpStatus.OK);
        }
        // Bad ID
